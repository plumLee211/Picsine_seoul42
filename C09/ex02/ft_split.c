...
/*   Updated: 2021/04/15 06:29:39 by jiyoulee         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int			count_words(char *str, char *charset)
{
	...
	cnt = 0;
	i = 1;
	if (is_target(str[0], charset) == 0)
		cnt++;
	while (str[i] != 0)
	{
		if (is_target(str[i], charset) == 0 &&
				is_target(str[i - 1], charset) == 1)
			cnt++;
		i++;
	}
	return (cnt);
...
char		**ft_split(char *str, char *charset)
{
	...
	{
		j = 0;
		while (is_target(str[index + j], charset) == 0)
			j++;
		if (j != 0)
		{
